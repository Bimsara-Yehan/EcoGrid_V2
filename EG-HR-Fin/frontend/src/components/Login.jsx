...
export default function Login() {
  ...
  const onSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Pseudo-login for demo: set local token and role, then go to dashboard
      localStorage.setItem('itp_token', '1');
      localStorage.setItem('itp_role', 'finance_manager');
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0f172a' }}>
      <form onSubmit={onSubmit} style={{ width: 360, padding: 24, border: '1px solid #334155', borderRadius: 8, background: '#111827', color: '#e5e7eb' }}>
        <h1 style={{ marginBottom: 16, fontSize: 20 }}>ITP Login</h1>
