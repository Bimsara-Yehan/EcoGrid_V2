...
export default function ConfirmModal({ open, date, drivers, onConfirm, onClose }: Props) {
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl border border-slate-200 w-full max-w-lg p-4">
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          <button className="px-3 py-2 rounded-lg bg-green-600 text-white font-semibold" onClick={onConfirm}>Confirm publish</button>
        </div>
      </div>
    </div>
  );
